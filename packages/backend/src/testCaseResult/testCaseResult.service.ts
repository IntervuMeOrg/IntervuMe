...
export const testCaseResultService = {
  ...
  async getByCodeSubmissionId(codeSubmissionId: string): Promise<TestCaseResultSchema[] | null> {
  ...
  async getByTestCaseId(testCaseId: string): Promise<TestCaseResultSchema[] | null> {
    const testCaseResults = await TestCaseResultRepository().find({
      where: { testCaseId },
      order: { created: 'DESC' }
    });
  ...
  async update(id: string, request: UpdateTestCaseResultSchema): Promise<TestCaseResultSchema | null> {
    const testCaseResult = await TestCaseResultRepository().findOne({
      where: { id }
    });
    ...
    Object.assign(testCaseResult, request, {updated: new Date().toISOString()});
